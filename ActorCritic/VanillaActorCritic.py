...
#         if total_step % UPDATE_EVERY == 0 or done:
#             if done:
#                 v_s_ = 0   # terminal
#             else:
#                 v_s_ = SESS.run(AC.v, {AC.s_batch: s_[np.newaxis, :]})[0,0]
...
#         ep_t += 1
#         if done:
#             GLOBAL_EPISODE_REWARD.append(ep_r)
#             if len(GLOBAL_RUNNING_REWARD) == 0:
#                 GLOBAL_RUNNING_REWARD.append(ep_r)
#             else:
#                 GLOBAL_RUNNING_REWARD.append(0.99 * GLOBAL_RUNNING_REWARD[-1] + 0.01 * ep_r)
#             if not env.unwrapped.lander.awake: solve = '| Landed'
#             else: solve = '| ------'
#             print(
#                 "Ep:", GLOBAL_EPISODES,
#                 solve,
#                 "| Ep_r: %i" % GLOBAL_RUNNING_REWARD[-1],
#                   )
#             GLOBAL_EPISODES += 1
#             # if not GLOBAL_EPISODES%SAVE_MODEL_EVERY :
#             #     saver.save(SESS, './my_test_model/ACmodel', global_step=GLOBAL_EPISODES)
#             break

# import pickle
# plt.plot(np.arange(len(GLOBAL_RUNNING_REWARD)), GLOBAL_RUNNING_REWARD)
# pickle.dump([GLOBAL_RUNNING_REWARD, GLOBAL_EPISODE_REWARD], open("rewards.pickle", "wb"))
# plt.xlabel('step')
...
# find state which donot need us to estimate Q(s,a), rather just need a sufficient V(s) which would say any action is fine?
# we want our agent to learn stochastic policy, hence we need it to have a certain degree of entropy built in, and not be entirely deterministic.

#V(s) here says from this state what is the best next state to be in is?
#like if we just fall from one pt to below, V(s) should learn 0, whereas if we take action of main thruster on, we'll receive -ve rewards.
#this will be counter productive to maximise rewards

#from this state what is the next best value to be in is?

#we can enact actor and a critic as 2 people in presentation?