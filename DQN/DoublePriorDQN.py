...
class DoublePrioritizedReplayDQN:
    def __init__(
            self,
            n_actions,
            n_features,
    ...
    def _build_net(self):
        ...
        with tf.variable_scope('eval_net'):
            c_names, n_l1, n_l2, w_initializer, b_initializer = \
                ['eval_net_params', tf.GraphKeys.GLOBAL_VARIABLES], self.hiddenunits[0], self.hiddenunits[1], \
    ...
    def choose_action(self, observation):
        ...
        if np.random.uniform() < self.epsilon:
            ...
            val = np.max(actions_value)
        else:
            action = np.random.randint(0, self.n_actions)
            val = actions_value[0, action]
        return action, val
